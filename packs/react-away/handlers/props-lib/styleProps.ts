...
export function addImportCSS(props: any, params: Params) {
   const isStyleRuleObject = x => Object.keys(x).includes('stylings')
   const module = global.own.modules.find(x => x.path == params.url)
   const styles = module.ports.imports.filter(isStyleRuleObject).map(x => x as any as StyleRule)
